...
function getHTTPSocket($url, $save_file = false, $file_name = false){
	...
	if($hostname !== false && $port !== false){
		if($scheme == "https" || $scheme == "http"){
			...
			if($scheme === "https"){
				$fp = fsockopen("ssl://".$hostname, $port, $err_no, $err_str, 5);
			}
			else if($scheme === "http"){
				$fp = fsockopen($hostname, $port, $err_no, $err_str, 5);
			}
    			...
    			if($save_file !== false && $file_name !== false){
    				...
    				if($filePointer){
    					...
    					if(preg_match("/^HTTP/", $response)){
    						...
    						while(!feof($fp)){
    							$response .= fgets($fp, SOCKET_CHUNK_SIZE);
    							if(strpos($response, "\r\n\r\n") !== false){
    								$response = explode("\r\n\r\n", $response);
    								$headers = $response[0];
    								$response = $response[1];
    								fwrite($filePointer, $response);
    						...
    						if(preg_match("/Content\-Length: (\d+?)\r\n/si", $headers, $match)){
    							...
    							while(!feof($fp)){
    								...
    								$downloaded_size += SOCKET_CHUNK_SIZE;
    							}
    						}
    					}
    					else{
